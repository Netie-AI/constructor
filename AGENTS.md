...
- Live run is `POST /cortex/constructor/run` from `engine.js` only when origin is `/cortex`. Ghost dry-run stays local. RSF-05: Constructor consumes CERTIFIED RSF into that path. Cortex mount note: `docs/patches/cortex-constructor-run-rsf.md`. Do not dual-seat Cortex (CREW-BELT-CLAIM).
